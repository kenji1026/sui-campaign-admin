...
export default function GasFeeReportPage() {
  ...
  useEffect(() => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/sponsor/stats/${dailyDate}`
    )
      .then((res) => res.json())
      .then((data) => {
        setStats(data);
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold mb-6">Gas Fee Report</h1>
      ...
      <div className="bg-white rounded p-6 shadow grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div>
          <div className="text-gray-500">Day Gas Fee Usage</div>
          <div className="text-2xl text-blue-700 font-mono">
            {formatCurrency(stats?.dayStats?.totalGas ?? 0)}
          </div>
        </div>
        <div>
          <div className="text-gray-500">Month Gas Fee Usage</div>
          <div className="text-2xl text-blue-700 font-mono">
            {formatCurrency(stats?.monthStats?.totalGas ?? 0)}
          </div>
        </div>
        <div>
          <div className="text-gray-500">Year Gas Fee Usage</div>
          <div className="text-2xl text-blue-700 font-mono">
            {formatCurrency(stats?.yearStats?.totalGas ?? 0)}
          </div>
        </div>
        <div>
          <div className="text-gray-500">Total Gas Fee Usage</div>
          <div className="text-2xl text-blue-700 font-mono">
            {formatCurrency(stats?.totalStats?.totalGas ?? 0)}
          </div>
        </div>
      </div>
      <div className="bg-white rounded p-6 shadow grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div>
          <div className="text-gray-500">Day Transactions</div>
          <div className="text-xl">
            {formatNum(stats?.dayStats?.totalTxs ?? 0)}
          </div>
        </div>
        <div>
          <div className="text-gray-500">Month Transactions</div>
          <div className="text-xl">
            {formatNum(stats?.monthStats?.totalTxs ?? 0)}
          </div>
        </div>
        <div>
          <div className="text-gray-500">Year Transactions</div>
          <div className="text-xl">
            {formatNum(stats?.yearStats?.totalTxs ?? 0)}
          </div>
        </div>
        <div>
          <div className="text-gray-500">Total Transactions</div>
          <div className="text-xl">
            {formatNum(stats?.totalStats?.totalTxs ?? 0)}
          </div>
        </div>
      </div>
    </div>
